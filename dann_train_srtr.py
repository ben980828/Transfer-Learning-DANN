...
class Target_Test_Image(Dataset):
    def __init__(self, fileroot, image_root, label_csv, transform=None):
        """ Intialize the MNIST dataset """
        self.images = None
        self.fileroot = fileroot
...
class Domain(nn.Module):
    ...
    def forward(self, input_data, epsilon):
        reverse_feature_extract = ReverseLayerF.apply(input_data, epsilon)#gradient update dir change
...
def main():
    ...
    target_set = Target_Test_Image(fileroot=target_root, 
    ...
    target_split, valid_split = random_split(dataset= target_set, lengths=[target_set_size, valid_set_size])

    source_loader = DataLoader(source_set, batch_size=256, shuffle=True, num_workers=1)
    source_batch = len(source_loader)
    valid_loader = DataLoader(valid_split, batch_size=700, shuffle=False, num_workers=1)
    target_loader = DataLoader(target_split, batch_size=int(len(target_split)/source_batch), shuffle=True, num_workers=1)
    ...
    optimizer = optim.Adam([{'params': feature_extractor.parameters()},
                            ...
                            {'params': domain.parameters()}], lr=lr, betas=(0.5, 0.999))
    ...
    for ep in range(1, epoch+1):
        ...
        for i, (source_data, target_data) in enumerate(zip(source_loader, target_loader)):
            ...
            optimizer.zero_grad()
            input_source, label_source, domain_source = source_data
            input_target, _, domain_target = target_data
            input_source, label_source, domain_source = input_source.cuda(), label_source.cuda(), domain_source.cuda()
            input_target, domain_target = input_target.cuda(), domain_target.cuda()

            src_feature = feature_extractor(input_source)
            tgt_feature = feature_extractor(input_target)

            class_pred = classifier(src_feature)
            class_loss = loss_class(class_pred, label_source)

            tgt_pred = domain(tgt_feature, eps)
            src_pred = domain(src_feature, eps)
            tgt_loss = loss_domain(tgt_pred, domain_target)
            src_loss = loss_domain(src_pred, domain_source)
        ...
        with torch.no_grad(): 
            for i, (data, class_label, domain_label) in enumerate(valid_loader):
                ...
                feature_eval = feature_extractor(data)
                class_output = classifier(feature_eval)
        ...
        if acc > max_acc:
            ...
            , 'dann_feature_extract_sr_{}_tr_{}_ep{}_acc{}.pth'.format(source_type, target_type, ep, acc))
            torch.save(classifier.state_dict()
            , 'dann_classifier_sr_{}_tr_{}_ep{}_acc{}.pth'.format(source_type, target_type, ep, acc))
            torch.save(domain.state_dict()
            , 'dann_domain_sr_{}_tr_{}_ep{}_acc{}.pth'.format(source_type, target_type, ep, acc))
    ...
    plt.savefig('Loss_Curve_DANN_Source_Target.png')
    # plt.show()

if __name__ == '__main__':
    main()

# model = Net(pretrain).to(device)
# optimizer = optim.Adagrad(model.parameters(), lr=0.001)
# load_checkpoint('/content/p2model-7500.pth', model, optimizer)
# model.eval()
# mask_transform('hw2-ben980828/hwvalid', model)