...
def main():
    ...
    target_loader = DataLoader(target_split, batch_size=128, shuffle=True, num_workers=1)
    ...
    for ep in range(1, epoch+1):
        ...
        for i, (data, class_label, domain_label) in enumerate(target_loader):
            p = float(i + ep * len(target_loader)) / epoch / len(target_loader)
            ...
            sys.stdout.write('\r epoch: %d, [iter: %d / all %d], class_loss: %f, domain_loss: %f' \
                % (ep, i + 1, len(target_loader), class_loss.data.cpu().numpy(),
        ...
        if acc > max_acc:
            ...
            max_acc = acc
            torch.save(dann.state_dict(), 'dann_nosr_tr_{}_ep{}_acc{}.pth'.format(target_type, ep, acc))
